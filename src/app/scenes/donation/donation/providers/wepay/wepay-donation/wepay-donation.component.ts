...
@Component({
  selector: 'donation-wepay',
  templateUrl: './wepay-donation.component.html',
  styleUrls: ['./wepay-donation.component.scss']
})
export class WepayDonationComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    //     };
      ...
      this.organisation.paymentMethods = this.organisation.paymentMethods.filter(pm => {
        if (pm.id === 'card') {
          this.currentSelectedPaymentMethod = pm;
          return true;
        } else {
          return false;
        }
