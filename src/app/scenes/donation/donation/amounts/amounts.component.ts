...
@Component({
    ...
    animations: [
        trigger('customAmountInputShownState', [
            ...
            transition('shown => not-shown', [style({
                display: 'block',
                opacity: 1
            }), animate('.15s', style({display: 'block', opacity: 0}))]),
            transition('not-shown => shown', [style({display: 'block'}), animate('0s .15s', style({
                display: 'block',
                opacity: 0
            })), animate('.15s', style({display: 'block', opacity: 1}))]),
        ])
    ]
})
